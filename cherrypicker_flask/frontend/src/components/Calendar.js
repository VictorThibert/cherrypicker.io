		...
		function drawChart() {
			...
			dayRects.enter().append('circle')
				...
				.attr('cy', function(d, i) {
					return MONTH_LABEL_PADDING + d.getDay() * (SQUARE_LENGTH + SQUARE_PADDING) + 10;
				})
				.on("mouseover", function(d, i) {
					if (countForDate(d) > 0 || countForDate(d) < 0) {
			...
			if (typeof onClick === 'function') {
				...
				dayRects.on('click', function(d) {
					if (countForDate(d) > 0 || countForDate(d) < 0) { //basically if a game was played that day
